...
### Experiment Params
###
QSO_SPEC_DIR     = "/Users/acm/Dropbox/Proj/astro/DESIMCMC/data/DR10QSO/specs"
NUM_BASES        = 4
BETA_VARIANCE    = 1.
BETA_LENGTHSCALE = 40.
...
if __name__=="__main__":
    ...
    # load all spec files
    spec_files = glob(os.path.join(QSO_SPEC_DIR, "spec*.fits"))
    spec_grid, spec_ivar_grid, spec_mod_grid, unique_lams, spec_zs, spec_ids, badids = \
        ru.load_specs_from_disk(spec_files)

    # cache 
    with open("qso_spec_data.pkl", 'wb') as handle:
        pickle.dump(spec_grid, handle)
        pickle.dump(spec_ivar_grid, handle)
        pickle.dump(spec_mod_grid, handle)
        pickle.dump(unique_lams, handle)
        pickle.dump(spec_zs, handle)
