...
$R_s$, the matrix encoding shape and rotation, and $z_s$, the red shift.
...
Now, we have the issue of where we should choose to birth a galaxy,
and on the flip side, which galaxy to choose to kill. Furthermore, we want
to choose reasonable values for each of the fluxes: perhaps, we should
be choosing this from some sort of prior.

\subsubsection{Merge-split for galaxies}

We derive each of the parameters.
For merging galaxy-mixing proportions, we set:
\begin{equation*}
\theta_s = \frac{1}{2}(\theta_1 + \theta_2)
\end{equation*}

When splitting, we sample random variable $g_1 \in [0, 1]$ and then
\begin{align*}
\theta_1 &= 2 g_1 \theta_s \\
\theta_2 &= 2(1 - g_1) \theta_s
\end{align*}
...
\begin{align*}
b_{j_1} & \leftarrow g_2 b_s\\
b_{j_2} & \leftarrow (1 - g_2) b_s
\end{align*}

For red-shift, when merging,
\begin{equation*}
z_s = \frac{1}{2}(\theta_1 + \theta_2)
\end{equation*}
...
TODO: How should we parametrize $R_s$ and merge/split it?
...
First, let us consider the star-to-galaxy transition. We randomly choose a mixing proportion $\theta \in [0, 1]$,
either uniformly or based on some other prior. We do the same for red-shift. The band-wise brightnesses
should be some function of the star's luminosity and temperature, perhaps, something like Planck's law
plus some randomness. The weights of the mixture components should also be chosen from a prior
to be decided. The matrix $R_s$ should be chosen randomly from the space of transformation matrices.

The important thing for the reverse transition is to make the reverse transition from band-wise brightnesses
to star brightness-temperature reversible.

TODO: Discuss and work out the math.
