...
\caption{Comparison of MCMC diagnostic statistics.  $\hat r$ refers to the potential scale reduction factor.  $N_{eff}$ is a measure of effective sample size based on between and within chain variance \cite{gelman2014bayesian}.  ESS is a measurement of effectively independent samples based on autocorrelation. Details can be found in \cite{gelman2011inference}.  We found parallel-tempering to consistently outperform a single chain, especially in our ``many-posterior'' tuning-free setting.}
\end{table}
