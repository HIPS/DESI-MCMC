...
Our model treats the spectral energy distribution (SED) of the radiation from a source as a latent variable that jointly explains both photometric and spectroscopic observations.  
...
Among these data are measurements of the spectral energy distributions (SEDs) of sources of light (e.g.~ stars, galaxies, and quasars).  
...
SEDs are of interesting because they convey information about a source's physical properties, including type, chemical composition, and redshift, which will be an estimand of interest in this work. 
...
This work develops a method for inferring physical properties sources by jointly modeling spectroscopic and photometric data.  
...
To jointly describe both resolutions of data, we directly model a quasar's latent SED and the process by which it generates spectroscopic and photometric observations. 
...
The SEDs of most stars are roughly approximated by Planck's law for black body radiators and stellar atmosphere models \cite{gray2001physical}. 
...
One of the most interesting properties of quasars (and galaxies) conveyed by the SED is redshift, which gives us insight into an object's distance and age.
...
Denoting the \emph{rest frame} SED of a quasar~$n$ as a function,~${f_n^{(\text{rest})} : \Lambda \rightarrow \mathbb{R}_+}$, the effect of redshift with value $z_n$ (typically between 0 and 7) on the \emph{observation-frame} SED is described by the relationship 
\begin{align}
...
\begin{figure*}[t]
\begin{center}
\centerline{\includegraphics[width=.9\textwidth]{../figs/quasar_redshift_example.png}}
\vskip -.16in
\caption{%
...
The SED of a quasar is a non-negative function~${f : \Lambda \rightarrow \mathbb{R}_+}$, where~$\Lambda$ denotes the range of wavelengths and~$\mathbb{R}_+$ are non-negative real numbers representing flux density.
...
The model assumes there are a small number ($K$) of latent features or characteristics and that each quasar can be described by a short vector of mixing weights over these features.  

We place a normalized log-Gaussian process prior on each of these basis functions (described in supplementary material).  
...
\begin{align}
  x_{n, \lambda} | z_n, \mathbf{w}_n, \{ B_k \} 
    &\stackrel{\textrm{ind}}{\sim} \mathcal{N}\left(\tilde f_n^{(\text{rest})}\left( \frac{\lambda}{1 + z_n} \right), \sigma_{n,\lambda}^2 \right)
    \label{eq:spec} 
\end{align}
...
Roughly, a photometric flux measures (proportionally) the number of photons recorded by the instrument over the duration of an exposure, filtered by a band-specific sensitivity curve. 
...
\end{align}
where $C(\lambda)$ is a conversion factor to go from the units of~$f_n(\lambda)$ to nanomaggies (details of this conversion are available in the supplementary material).
...
%where $\mu_b$ to represent the full function from a rest spectrum and red shift to a band-specific flux.
The function $\mu_b$ takes in a rest frame SED, a redshift ($z$) and maps it to the observed $b$-band specific flux.  
...
For photometric weight and redshift inference, we use a flat prior on $z_n \in [0, 8]$, and empirically derived priors for $m_n$ and $w_n$, from the sample of spectroscopically measured sources.  Choice of priors is described in the supplementary material. 
...
\end{align}
We use gradient descent with momentum and LBFGS \cite{nocedal1980updating} directly on the parameters $\beta_k, \omega_{n,k}$, and $\log(m_n)$ for the $N_{spec}$ spectroscopically measured quasars.  Gradients were automatically computed using \texttt{autograd} \cite{maclaurin2015autograd}.  
...
To compute these distributions, we construct a Markov chain over the state space including $z$, $\mathbf{w}$, and $m$ that leaves the target posterior distribution invariant.
...
Note that due to analytic intractability, we numerically integrate expressions involving $\int_\Lambda f_n^{(obs)}(\lambda) d\lambda$ and $S_b(\lambda)$.
...
Single chain MCMC methods have to jump between modes or travel through a region of near-zero probability, resulting in slow mixing.  
...
\paragraph{Basis validation} We examined multiple choices of $K$ using out of sample likelihood on a validation set.  In the following experiments we set $K=4$, which balances generalizability and computational tradeoffs.  Discussion of this validation is provided in the supplementary material. 
...
{
  \hskip -.2in \includegraphics[width=.55\textwidth]{../figs/rank_4_basis-random.png}
}
\includegraphics[width=.99\textwidth]{../figs/idx_4_rank_4_reconstruction.png}
\vskip -.2in
\end{figure}
...
Basis $B_4$ places mass on the Lyman-$\alpha$ peak around 1,216~$\angstrom$, allowing the model to capture the co-occurrence of more peaked SEDs with a bump around 1,550~$\angstrom$.  
Basis $B_1$ captures the H-$\alpha$ emission line at around 6,500~$\angstrom$.  
Because of the flexible nonparametric priors on $B_k$ our model is able to automatically learn these features from data. 
...
In general there is a strong correspondence between spectroscopically measured redshift and our posterior estimate.
...
To combat this multi-modality, it is necessary to inject the model with more information to eliminate plausible hypotheses; this information could come from another measurement (e.g., a new photometric band), or from structured prior knowledge over the relationship between $z_n, \mathbf{w}_n$, and $m_n$.  Our method simply fits a mixture of Gaussians to the spectroscopically measured $\mathbf{w}_n, m_n$ sample to formulate a prior distribution.  However, incorporating dependencies between $z_n$, $\mathbf{w}_n$ and $m_n$, similar to the XDQSOz technique, will be incorporated in future work. 


%----- Red Shift, spec vs photo -------------------------------------------------
\begin{figure}[t]
\begin{center}
  \includegraphics[height=.34\textwidth]{{scatter_exp-random}.png}
  \includegraphics[height=.34\textwidth]{{scatter_exp-flux}.png}
  \includegraphics[height=.34\textwidth]{{scatter_exp-redshift}.png}
\end{center}
\vskip -.16in
\caption{%
...
\begin{figure*}[t]
\centerline{
\includegraphics[height=.21\textwidth]{../figs/quasar_plots/quasar_490_mcmc_recon.png}
\includegraphics[height=.21\textwidth]{../figs/quasar_plots/quasar_490_posterior_z.png}
} 
\centerline{
\includegraphics[height=.21\textwidth]{../figs/quasar_plots/quasar_214_mcmc_recon.png}
\includegraphics[height=.21\textwidth]{../figs/quasar_plots/quasar_214_posterior_z.png}
}
\caption{Left: inferred SEDs from photometric data.  
The black line is a smoothed approximation to the ``true'' SED using information from the full spectral data.  
...
One disadvantage to this approach is there there is no physical significance to the mixture of Gaussians, and no model of the latent SED.  
...
The regression approach from \cite{brescia2013photometric} employs a neural network with two hidden layers, and the SDSS fluxes as inputs. 
...
Additionally, because we are directly modeling the latent SED, our method admits a posterior estimate of the entire SED.
...
%----------------------------------------------------------------------------------
\section{Discussion}
We have presented a generative model of two sources of information at very different spectral resolutions to form an estimate of the latent spectral energy distribution of quasars.
We also described an efficient MCMC-based inference algorithm for computing posterior statistics given photometric observations. 
...
%sOur current specification uses the output of a photometric preprocessing step, however, a logical extension is to model photon counts at the pixel level and perform inference over a joint model. 
%sBecause pixel appearance models conditioned on the SED of a source are well-known, we see this as a feasible next step. 
...
%sModels for spatial extent have been developed and applied to photometric data \cite{hogg2013replacing, regier2015}.  
