...
Section~\ref{sec:related} reviews related work on the ``photo-z'' problem.  
...
\label{sec:gps}
Due to the complicated shape of quasar SEDs, we use a Gaussian process (GP) prior to flexibly encode our prior beliefs about their structure and shape. 
...
%----- -------------------------------------------------
\begin{figure}[t]
\floatbox[{\capbeside\thisfloatsetup{capbesideposition={left,top},capbesidewidth=4.5cm}}]{figure}[.5\FBwidth]
{ 
  \caption{%
...
}
{
  ...
  \includegraphics[width=.55\textwidth]{../figs/idx_4_rank_4_reconstruction.pdf}
}
\vskip -.16in
\end{figure}
...
The fact that the SED-based model is consistent with the physical world suggests that we should be able to make better predictions for quasars with very different properties from the training set, e.g, distance. Indeed, we see that it does.  
