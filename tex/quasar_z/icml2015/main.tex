...
\begin{abstract} 
We describe a joint model of two disparate sources of astronomical data, spectroscopy and photometry, which carry information about sources light at extremely different resolutions. 
Our model treats the energy distribution of a source's radiation as a latent variable, which hierarchically generates both photometric and spectroscopic observations.  
...
We place a structured stochastic process prior over the spectral energy distribution of a light source (e.g.~star, galaxy, or quasar) that admits a physically interpretable decomposition and allows us to tractably perform fully Bayesian inference using Markov Chain Monte Carlo (MCMC).  
We use our model to compute the distribution of red-shift of quasars from five-band (low-resolution) photometric data, the so called photometric red-shift estimation (``photo-z'') problem. 
