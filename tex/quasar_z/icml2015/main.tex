...
\begin{abstract} 
We describe a joint model of two different sources of astronomical data, spectroscopy and photometry, which carry information about a source of light at extremely different resolutions. 
...
Our method treats the unknown spectral energy distribution of an object (e.g.~star, galaxy, or quasar) as a latent stochastic process and uses statistical inference to infer the unknown structure. 
We use our model to infer the red-shift of quasars from five-band (low resolution) photometric data, the so called ``photo-z'' problem. 
...
Enormous amounts of astronomical data are collected \red{cite sloan digital sky survey, other astronomical surveys}.  Among this collection are measurements of spectral energy distribution (WED) of a light source (e.g.~a star, galaxy, quasar).  
