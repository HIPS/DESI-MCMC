...
\begin{align}
  y_{n,b} | \mathbf{w}_n, z_n, m_n, B &\sim \mathcal{N}( I_b(f_n^{(\text{obs})}, z_n), \tau_{n,b} ) \, .
\end{align}
...
Due to analytic intractability, we numerically integrate expressions $\int_\Lambda f_n^{(obs)}(\lambda) d\lambda$. 
...
The posterior distribution from which we sample, $p(B | \mathbf{X})$, can be written 
\begin{align}
  p(B | \mathbf{X}) 
    &= L(\beta; \mathbf{X}) p(\beta) \\
    &= L(\beta; \mathbf{X}) \prod_{k} \mathbf{N}(\beta_k | 0, K_{\theta_k}) \, .
\end{align}
We draw posterior samples using Hamiltonian Monte Carlo \cite{neal2011mcmc}, an auxiliary variable method that uses gradient information to efficiently mix.  Empirically, naive Metropolis-Hastings empirically mixes too slowly for practical purposes. 
...
\hline
\abovespace
MAE  &  0.4278 & 0.2527 & 0.2297 & 0.2127 & 0.0893\\
MAPE &  0.3966 & 0.1134 & 0.0895 & 0.0634 & 0.0221 \\
\hline
\% in $[.05, 99.5]$ &  \\ %0.3543 & 0.1359 & 0.1132 & 0.0781 & 0.0221 \\
\% in $[2.5, 97.5]$ \\
\hline
\end{tabular}
\end{sc}
...
Using information only from $M = 400$ noisy high resolution SED measurements, our structured prior is able to accurately predict and characterize uncertainty about the red-shift of photometrically measured quasars.  
...
\begin{itemize}
  ...
  \item incorporation of new physical characteristics (distance, dust, etc)
